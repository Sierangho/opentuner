...
void DoFilter(void)
{
	...
	//my tiling
	uint32_t horTileSize = 1024;
	uint32_t verTileSize = 1024;
	...
	//remember filter rect is setup by the host
	sprintf(stringVal, "filter rect init: %d,%d,%d,%d\n", filterRect.top, filterRect.bottom, filterRect.left, filterRect.right);
	logIt.Write(stringVal, false);
	...
	for (int32 y = 0; y < verTiles; y++){
		for (int32 x = 0; x < horTiles; x++){
			...
			for (int16 plane = 0; plane < gFilterRecord->planes - 1; plane++)
			{
				...
				*gResult = gFilterRecord->advanceState();
				...
				sprintf(stringVal, "row %d,%d", gFilterRecord->outRowBytes, gFilterRecord->inRowBytes);
				logIt.Write(stringVal, true);

				uint8_t * bytes = (uint8_t *)gFilterRecord->inData;

				//do_blur_test(inRect.right,inRect.bottom,(uint32_t)gFilterRecord->inData,(uint32_t)gFilterRecord->outData, plane);
				VRect filterRect = GetFilterRect();
				//sprintf(stringVal, "filterRect %d,%d,%d,%d", filterRect.top, filterRect.bottom, filterRect.left, filterRect.right);
				//logIt.Write(stringVal, true);
				...
				do_blur_test(input_width, input_height, output_width, output_height, input_scanline, output_scanline, (uint32_t)gFilterRecord->inData, (uint32_t)gFilterRecord->outData, plane);
				

				/*for (int i = 0; i < verTileSize + 2 * verPadding ; i++){
					for (int j = 0; j < horTileSize + 2 * horPadding; j++){
						sprintf(stringVal, "%d,%d,%d", j, i, bytes[i * (gFilterRecord->inRowBytes) + j]);
						logIt.Write(stringVal, true);
					}
				}*/
				uint32_t val = -1;
				sprintf(stringVal, "done %d %d %d\n", plane, gFilterRecord->inputPadding, val);
				logIt.Write(stringVal, true); 
